...
SELECT staffs.staff_id, staffs.first_name, staffs.last_name, SUM(order_items.quantity) AS [number_bike]
FROM orders
JOIN order_items ON order_items.order_id = orders.order_id
JOIN staffs ON orders.staff_id = staffs.staff_id
WHERE staffs.staff_id = 
(SELECT TOP 1 staffs.staff_id FROM orders
        JOIN order_items ON orders.order_id = order_items.order_id
        JOIN staffs ON orders.staff_id = staffs.staff_id
        WHERE YEAR(orders.order_date) = 2017 AND MONTH(orders.order_date) BETWEEN 4 AND 6
        GROUP BY staffs.staff_id
        ORDER BY SUM(order_items.quantity) ASC)
    AND YEAR(orders.order_date) = 2017
    AND MONTH(orders.order_date) BETWEEN 4 AND 6
GROUP BY staffs.staff_id, staffs.first_name, staffs.last_name
